...
function renderProfileView(user) {
  ...
  document.getElementById("profile-avatar-lg").textContent = (user.name || "U")
    .split(" ")
    .map((n) => n[0])
    .slice(0, 2)
    .join("")
  ...
  grid.innerHTML = vendors.length
    ? vendors
      ...
      <div class="vendor-body">
        ...
        .join("")
    : '<div class="empty-state"><h3>You are not following any vendors yet</h3><p>Follow vendors to see them here.</p></div>';
}
...
function initProfileToggle(user) {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const savedUser = response.data || response.user || response;
      Object.assign(user, savedUser, updates);
      renderProfileView(user);
      ...
      else console.error("Could not update profile:", error);
    }
  });
}
