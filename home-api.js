    ...
    try {
      ...
      const products = Array.isArray(response.products)
        ...
        : Array.isArray(response.data)
          ? response.data
          : [];
      carousel.replaceChildren(...products.map(createCarouselCard));
      if (!products.length)
        carousel.innerHTML =
          '<p class="empty-state">No product images available yet.</p>';
    } catch (error) {
      carousel.innerHTML = `<p class="empty-state">${error.message || "Could not load product images."}</p>`;
    }
  }
      ...
      document
        ...
        .forEach((item) =>
          item.classList.toggle(
            "active",
            item === card && Boolean(activeCategory),
          ),
        );
      loadProducts();
    });
...
function createCarouselCard(product) {
  ...
  card.innerHTML = `<div class="carousel-image">${image ? `<img src="${image}" alt="${product.name || "Product"}">` : "<span>No image</span>"}</div><div class="carousel-card-info"><span>${product.category || "Local product"}</span><strong>${product.name || "Product"}</strong><small>${product.vendor_name || product.vendor?.name || ""}</small></div>`;
  return card;
}
