<script setup>
import { ref } from 'vue';
import { apiFetch, assetUrl, getAuthToken, money } from '../../app/services/api';
import { showError } from '../../app/services/alerts';
const props = defineProps({ product: { type: Object, required: true } }); const added = ref(false);
const image = assetUrl(props.product.image_url || props.product.image || props.product.imageUrl || props.product.image_path || props.product.photo_url);
