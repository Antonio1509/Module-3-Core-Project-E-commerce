  ...
  if (!outOfStock && btn) {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (typeof showToast === "function")
          showToast(
            error.message || "Could not add this product to your cart.",
          );
        else console.error("Could not add product to cart:", error);
      }
    });
  }
  return card;
...
async function renderVendor() {
  ...
  try {
    ...
    const banner =
      vendor.cover_image ||
      vendor.cover ||
      "https://images.unsplash.com/photo-1494526585095-c41746248156?w=800&q=60";
    const logoText =
      vendor.logo_text ||
      (vendor.name || "LC")
        .split(" ")
    ...
    document.getElementById("store-banner").style.backgroundImage =
      `url('${banner}')`;
    document.getElementById("store-logo").textContent = logoText;
    document.getElementById("store-category").textContent =
      vendor.category || "Local vendor";
    ...
    let followingCount = Number(
      vendor.following_count ?? vendor.following ?? 0,
    );
    const vendorFollowerValue =
      vendor.follower_count ?? vendor.followers_count ?? vendor.followers;
    const storedFollowerCount = localStorage.getItem(
      `localcart-vendor-followers-${id}`,
    );
    let followerCount =
      vendorFollowerValue == null
    ...
    const refreshRatingFromProducts = async () => {
      const reviewResponses = await Promise.all(
        products.map((product) =>
          apiFetch(
            `/products/${encodeURIComponent(product.id || product.product_id)}/reviews`,
          ),
        ),
      );
    ...
    try {
      await refreshRatingFromProducts();
    } catch (error) {
      console.error("Could not load vendor reviews:", error);
    }
    renderStats();
    ...
    window.addEventListener("localcart:review-saved", (event) => {
      ...
      if (
        products.some(
          (product) => Number(product.id || product.product_id) === productId,
        )
      ) {
        refreshRatingFromProducts()
          .then((refreshed) => {
            if (refreshed) return;
    ...
    document.getElementById("about-text").textContent =
      vendor.about ||
      vendor.description ||
      "This vendor has not added a storefront bio yet.";
    ...
    document.getElementById("info-response").textContent =
      vendor.response_time || "Usually replies within a day";
    document.getElementById("info-delivery").textContent =
      vendor.delivery_area || "Nationwide";
    document.getElementById("info-shipping").textContent =
      vendor.shipping_info || "Standard shipping";
    document.getElementById("info-joined").textContent =
      vendor.joined || "Recently";
    ...
    if (followBtn) {
      ...
      if (user) {
        ...
        following = followedVendors.some(
      ...
      followBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          await apiFetch(`/users/following/${encodeURIComponent(id)}`, {
            method,
          });
          following = !following;
