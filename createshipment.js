document.addEventListener('DOMContentLoaded', async () => {
  ...
  const render = () => {
    ...
    const filtered = orders.filter(order => `${order.number} ${order.customer} ${order.email} ${order.method} ${order.destination}`.toLowerCase().includes(query));
    list.innerHTML = filtered.map(order => `
      ...
      </article>`).join('') || '<div class="empty">No unshipped customer orders match your search.</div>';
  };
  ...
  list.addEventListener('click', async event => {
    ...
    if (option.dataset.option === 'rankdrop') {
      pickupPointId = Number(localStorage.getItem('pickupPointId'));
      if (!pickupPointId) {
        alert('Choose a RankDrop pickup point first.');
    ...
    try {
      await apiFetch('/shipments', {
        method: 'POST',
        body: JSON.stringify({ orderId: Number(option.dataset.order), method: option.dataset.option, pickupPointId })
      });
      alert('Shipment created successfully.');
      orders = orders.filter(order => Number(order.orderId || order.id) !== Number(option.dataset.order));
      render();
    } catch (error) {
